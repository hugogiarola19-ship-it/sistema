...
export const Route = createFileRoute("/projects")({
  head: () => ({
    meta: [
      { title: "Projetos — Giarola Engenharia" },
      { name: "description", content: "Gestão de projetos do escritório." },
    ],
  }),
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <Select value={status} onValueChange={(v) => setStatus(v as any)}>
          ...
          <SelectContent>
            ...
            {(
              ["Em andamento", "Aguardando cliente", "Entregue", "Arquivado"] as ProjectStatus[]
            ).map((s) => (
        ...
        <Select value={type} onValueChange={(v) => setType(v as any)}>
          ...
          <SelectContent>
            ...
            {(["Unifamiliar", "Multifamiliar", "Comercial"] as ProjectType[]).map((s) => (
              <SelectItem key={s} value={s}>
                {s}
              </SelectItem>
            ))}
            ...
            <Card key={p.id} className="hover:shadow-sm transition-shadow">
              <CardContent className="p-4 sm:p-5">
                ...
                <div className="mt-3 flex items-center gap-3">
                  <Progress value={projectProgress(tasks, sections, p.id)} className="h-2 flex-1" />
