      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {client.personType === "PJ" ? "Pessoa Jurídica" : "Pessoa Física"} · {client.type} ·{" "}
            {client.city}
          </p>
          {(client.document || client.responsible) && (
            <p className="text-xs text-muted-foreground mt-1">
              {client.document && (
                <>
                  {client.personType === "PJ" ? "CNPJ" : "CPF"}: {client.document}
                </>
              )}
          ...
          <CardContent className="text-sm space-y-2">
            ...
            {client.notes && (
              <div className="pt-3 border-t mt-3">
                <p className="text-xs uppercase text-muted-foreground mb-1">Observações</p>
          ...
          <CardContent className="space-y-3">
            ...
            {linked.map((p) => (
              <Link
                ...
                to="/projects/$id"
                params={{ id: p.id }}
                className="block rounded-lg border p-3 hover:bg-secondary/40"
              >
