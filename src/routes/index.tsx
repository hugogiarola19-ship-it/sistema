...
import { useProjects, useClients, useTasks, useTransactions, projectProgress, formatBRL, formatDate } from "@/lib/storage";
...
function Dashboard() {
  ...
  const active = projects.filter(p => p.status === "Em andamento" || p.status === "Aguardando cliente");
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4 mb-6">
        <Metric icon={FolderKanban} label="Projetos ativos" value={String(active.length)} hint={`${projects.length} no total`} />
        <Metric icon={CalendarClock} label="Prazos próximos" value={String(upcoming)} hint="Próximos 14 dias" />
        <Metric icon={TrendingUp} label="Receita do mês" value={formatBRL(received)} hint={`Previsto ${formatBRL(expected)}`} />
        <Metric icon={Users} label="Clientes ativos" value={String(activeClients)} hint={`${clients.length} cadastrados`} />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-4">
            {active.length === 0 ? (
              <EmptyState icon={FolderKanban} title="Nenhum projeto ativo" action={<Button asChild><Link to="/projects">Criar primeiro projeto</Link></Button>} />
            ) : active.map(p => (
              <Link key={p.id} to="/projects/$id" params={{ id: p.id }} className="block rounded-lg border p-4 hover:bg-secondary/40 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-medium truncate">{p.name}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{clientName(p.clientId)} · prazo {formatDate(p.deadline)}</p>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {weeklyTasks.length === 0 && <p className="text-sm text-muted-foreground">Sem tarefas marcadas.</p>}
              {weeklyTasks.map(t => {
                ...
                return (
                  <label key={t.id} className="flex items-start gap-3 text-sm cursor-pointer">
                    <Checkbox
                      checked={done}
                      onCheckedChange={(c) => updateTask(t.id, { status: c ? "Concluído" : "A fazer" })}
                      className="mt-0.5"
                    />
                    <span className={done ? "line-through text-muted-foreground" : ""}>{t.title}</span>
                  </label>
                );
              })}
            </CardContent>
...
function Row({ label, value, bold, accent }: { label: string; value: string; bold?: boolean; accent?: "success" | "warning" | "destructive" }) {
